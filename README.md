...
浏览器使用独立上下文，不会沿用用户已打开的 Chrome 登录状态。同一时间只能运行一个测试。默认运行时限为 120 秒加用例中的条件等待时限；显式设置的运行时限仍优先。
...
本机只保存用例与标签页的临时连接信息，不复制登录 Cookie。为识别 Chrome 标签页会话是否已更换，开始录制或确认会话时在该网站的 sessionStorage 写入 `__testing_workspace_bridge_session` 随机标记；它不是认证凭证。标签页关闭、更换窗口、离开目标网站或标记失效时，回放会报错，不会自动接管其他标签页。业务截图、输入内容和步骤仍按已有录制规则保存，请在登录结束后再开始录制。
