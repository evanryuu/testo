...
「按录制操作回放」使用录制坐标和官方 Midscene actionSpace，不请求模型。独立浏览器自动采用录制尺寸（没有尺寸信息时默认 1280 × 800）；连接现有 Chrome 时也会设置录制的网页内容尺寸，并在每个坐标操作前检查稳定性。每步可以切换为「AI 描述执行」，生成 `aiAct`；新生成的纯 AI 用例不再添加视口限制，混合用例保留坐标步骤需要的尺寸检查。尺寸相同也不保证动态布局完全相同。

断言可选「页面包含可见文本」或「AI 判断预期结果」。前者检查页面中精确匹配的可见文本，后者生成 `aiAssert` 并调用模型。没有断言时，Passed 仅代表操作执行完成，不能证明业务结果正确。

Workspace 生成的 `recordedAction`、`assertText` 和 `requireViewport` 是本项目为 `@midscene/test` 注册的节点；直接交给其他 Midscene CLI 执行前，需要同时接入这些节点，或改成标准 AI 步骤。初始地址使用 `${baseUrl}`，后续手动输入的跳转地址保留录制时的完整 URL。
...
1. 在桌面 Chrome 安装/启用 Midscene 扩展，打开所选环境的网站，并保持该标签页为 Chrome 当前标签页。
2. 在用例详情的「浏览器会话」选择「Chrome 现有会话」，点击「连接 Chrome」。在扩展提示中允许 Bridge 连接；其他 Midscene Bridge 会话需先断开。
3. Workspace 显示「等待手动登录」。在已经连接的 Chrome 标签页完成登录，并返回目标业务页面。此阶段不创建预览、不采集截图/步骤，也不主动附加 Midscene debugger。
4. 回到 Workspace 点击「已准备好，开始录制」，然后在 Workspace 的官方预览中进行业务操作。原生 Chrome 内的人工操作不会进入 Timeline。
5. 停止录制、检查并保存，保持「Chrome 现有会话」选项再点击「运行」。运行重连录制时的标签页；其他标签页成为前台不会改变执行目标。
6. 已有 Workflow 时，连接后可以直接点击「登录完成，返回用例运行」，无需重新录制。Workspace 重启后需要重新连接；不会从磁盘自动恢复 Chrome 控制权限。

结束、取消或退出 Workspace 只断开连接，不关闭用户 Chrome、标签页或清除 Cookie。用例自身的退出登录等操作仍会影响真实会话。未登录用例选择「独立会话（未登录）」。

登录是否完成由用户在准备阶段确认，本版本不识别各业务站点的认证状态，也不能保证安全验证通过。登录失效后应停止操作，重新连接、手动登录；暂不提供检测会话过期并自动续登。测试期间不修改服务端权限或会员状态。
...
在用例列表点击「批量运行」，选择运行环境和多个已保存的 Web 用例，调整执行顺序，为每个用例选择登录窗口。

先在 Chrome 中打开所选环境的网站并手动登录，保持目标标签页为 Chrome 当前标签页，然后回到 Workspace 为窗口命名并点击「确认当前登录窗口」。确认过程不导航、不录制；按相同步骤可以添加多个窗口。同一 Chrome profile 的普通窗口共享 Cookie，多个窗口不代表多个独立账号；当前批次使用同一扩展连接，不支持不同 profile 并行运行。

Midscene 扩展默认在后台监听连接请求；要自动连续运行，可在官方连接提示中选择「Always Allow」，否则每次重连仍需确认。平台不会修改扩展的授权设置。参见 [官方 Bridge 说明](https://v1.midscenejs.com/bridge-mode)。

所有窗口共用一条顺序队列。每例重新连接已确认的标签页，运行自己的起始页面和步骤，完成清理后才开始下一例。默认遇到失败停止，可选择失败后继续。取消批次会停止当前用例并跳过剩余用例。页面显示每项状态和独立运行详情；批次及单例报告都保留在运行历史中。

应用重启后保留批次历史，未完成的批次标记为中断，不自动恢复执行。登录窗口需要重新确认。排队期间 Workflow 如有修改，该项会报错，需要重新发起批次。用例内的退出登录、页面操作会影响共享会话；每例仍需明确等待条件和断言。
...
录制结束后，可以在 Timeline 的任意操作前后插入“等待条件”或“断言”，修改内容和等待上限，也可以上下移动或删除检查步骤。预览和保存的 YAML 按 Timeline 顺序执行；原始录制事件及其截图保持不变，页面自动产生的导航记录仍只作为观察证据。旧草稿中末尾的等待和断言会保留在末尾；编辑后的顺序会保存在本机。已有用例也可以从详情的 Web“查看 / 编辑”修改 YAML，无需重新录制。等待步骤默认最多 60 秒，支持 1–300 秒；条件提前满足立即继续。AI 断言仍只检查当时页面，不会因为描述里写了“等十秒”而延迟执行。
...
| 已注册 YAML，无独立表单 | aiTap、aiBoolean、aiNumber、aiString、aiAsk、固定时长 wait |
...
这是一份代码接入清单，不代表每个只通过 YAML 暴露的节点都完成了真实模型验证。已知限制：当前保存校验采用独立浏览器节点集，Cookie/视口节点可能通过保存但在 Chrome Bridge 运行失败；高级 agent 节点也会因缺少执行器失败。这两项没有在本次等待功能中扩展修复。
