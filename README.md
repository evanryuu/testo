...
批量运行支持逐用例选择数据集、共享参数、每例总时限及可选登录条件。开始前检查全部用例，保存本批次的定义和配置快照。历史中可以重跑全部、失败或未完成的项，并重新选择本次 Chrome 标签页。每次重跑都会重新读取最新保存的 YAML 和项目共享步骤，保留原批次的用例顺序、数据集选择和运行配置；共享参数可以在重跑时调整。当前脚本无效、文件缺失、数据集或共享步骤被删除时，会在启动前报错。重跑生成新的执行快照，不修改旧批次的历史记录；开始后再修改脚本不影响已经排队的本次执行。
