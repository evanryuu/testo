...
批量运行支持逐用例选择数据集、共享参数、每例总时限及可选登录条件。开始前检查全部用例，保存本批次的定义和配置快照。历史中可以重跑全部、失败或未完成的项，并重新选择本次 Chrome 标签页。每次重跑都会重新读取最新保存的 YAML 和项目共享步骤，保留原批次的用例顺序、数据集选择和运行配置；共享参数可以在重跑时调整。当前脚本无效、文件缺失、数据集或共享步骤被删除时，会在启动前报错。重跑会更新当前批次，按每个用例的最新结果聚合，保留未重跑用例的结果；全部用例通过后，批次才显示通过。历史列表显示通过数量和重跑次数，批次详情可切换首次运行和第几次重跑，查看各次独立的脚本、配置快照和单例报告。开始后再修改脚本不影响已经排队的本次执行。只有配置新批次才会新增批次记录；旧版本已经生成的独立批次不会自动合并。

创建、改名、删除等前后依赖的用例应勾选“关联场景”。此类批次重跑时从首项开始，避免只运行删除等依赖步骤。普通批次可以只重跑失败或未完成项；旧批次缺少运行配置快照时会明确拒绝，需要配置新批次。
