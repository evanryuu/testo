  ...
  return <Dialog open onOpenChange={open => { if (!open && !busy) close(); }}><DialogContent showCloseButton={false} className="max-h-[92vh] overflow-hidden sm:max-w-5xl" onInteractOutside={event => event.preventDefault()}>
    ...
    <div className="min-h-0 space-y-4 overflow-y-auto pr-1">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-3 rounded-lg border p-4"><div className="flex items-center justify-between gap-2"><h3 className="text-sm font-medium">已选成员与顺序 · {draft.caseIds.length}</h3><Button type="button" size="sm" variant="ghost" disabled={busy || !draft.caseIds.length} onClick={() => setDraft(current => ({ ...current, caseIds: [] }))}>清空已选成员</Button></div><div className="max-h-96 divide-y overflow-y-auto rounded border">{draft.caseIds.slice(currentSelectedPage * PAGE, (currentSelectedPage + 1) * PAGE).map((id, localIndex) => {
